...
export interface PackagedTextFiles {
  name: string;
...
/** Package exact text bytes in a deterministic, uncompressed, flat ZIP. */
export const packageTextFiles = (name: string, files: readonly TextFile[]): PackagedTextFiles => {
  ...
  for (const file of files) {
    ...
    local.view.setUint16(26, encodedName.length, true);
    local.view.setUint16(28, 0, true);
    localChunks.push(local.bytes, encodedName, content);
    ...
    central.view.setUint16(28, encodedName.length, true);
    ...
    centralChunks.push(central.bytes, encodedName);
    localOffset += local.bytes.length + encodedName.length + content.length;
  }
  ...
  return {
    name,
    mimeType: "application/zip",
    value: concatBytes([...localChunks, centralDirectory, end.bytes]),
  };
};
