---
title: "Fieldset"
group: "Forms"
tags: ["fieldset"]
kind: "native"
purpose: "Group of related form controls."
treatment: "Keep native fieldset semantics; style with Framework tokens without removing browser fallback behavior."
use: ["Use legend as first child for shared question/context."]
avoid: "Use only for visual border."
version: "0.1.0"
baseline: { status: "widely-available", source: "mdn", sourceUrl: "https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/fieldset", checkedAt: "2026-07-16" }
deprecated: false
order: 640
sourceUrl: "https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/fieldset"
---

<div class="native-demo"><fieldset><legend>Preferred output</legend><label><input type="radio" name="output-demo" checked> HTML</label><label><input type="radio" name="output-demo"> React</label></fieldset></div>
