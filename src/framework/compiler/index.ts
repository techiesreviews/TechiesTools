...
export const compileFramework = (input: CompileFrameworkInput): FrameworkCompilation => {
  ...
  const contextBlocked = cssContextBlocked || blockingContextDiagnostics.length > 0;
  ...
  return deepFreeze({
    ...
    artifacts: {
      ...
      context: contextBlocked ? block([...primitiveDiagnostics, ...blockingElementDiagnostics, ...blockingContextDiagnostics]) : { available: true, value: contextArtifact },
    },
    diagnostics,
