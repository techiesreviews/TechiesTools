...
One dynamic authoring route composes the shared Settings bar and Preview browser for all Patterns. Each package supplies distinct Appearance controls. A Pattern-only Advanced action opens the reusable bottom drawer with editable, separate panels for the complete portable HTML and stylesheet; the drawer stays closed outside that explicit task. Appearance controls remain shortcuts over the root declaration list within the same stylesheet state. Valid edits update and persist the Preview and its footer export artifacts; invalid drafts keep the last valid rendering visible. Reset remains in the protected Settings header and restores that package's canonical Framework-backed sources.
